...
def login():    
    if request.method == 'POST':
        ...
        if user:
            ...
            print("SET SESSION USER ID:", session['user_id'])
...
def history():
    ...
    if session.get('role') == 'admin':
        cursor.execute("""
            SELECT rentals.id, users.username, cars.name, rentals.status, rentals.due_date
            FROM rentals
        ...
        cursor.execute("""
            SELECT rentals.id, cars.name, rentals.status, rentals.due_date
            FROM rentals
            JOIN cars ON rentals.car_id = cars.id
